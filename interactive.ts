...
function convertToArrayOfAliases(str: string) : string[] {
    const arr: string[] = [];
...
const processEncoding = (str: string) : boolean => {
    try {
        log(`Encoded: ${encode(str)}`);
        return true;
    } catch (ignored) {
        return false;
    }
...
const processDecoding = (str: string) : boolean => {
    try {
        log(`Decoded: ${decode(str)}`);
        return true;
    } catch (ignored) {
        return false;
    }
...
declare global {
    interface String {
        equalsIc(...str: string[]) : boolean;
    }
}

String.prototype.equalsIc = function(...str: string[]) : boolean {
    for (const s of str)
...
const isStatePrepared = () : boolean =>
    (currentMode !== '' && currentInput.trim() !== '')

function gatherInput() {
    if (currentMode === '') {
        const input: string = prompt('Are you encoding or decoding? ');
